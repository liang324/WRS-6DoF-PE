...
class PyKinectAzure(object):
    ...
    def device_get_serialnum(self):
        ...
        ::K4A_BUFFER_RESULT_TOO_SMALL without returning any log_filename in serial_number.
    ...
    def device_start_cameras(self, device_config=None):
        ...
        Individual sensors configured to run will now start to stream captured log_filename..
    ...
    def device_start_imu(self):
        ...
        Call this API to start streaming IMU log_filename. It is not valid to call this function a second time on the same
    ...
    def device_get_capture(self, timeout_in_ms=_k4a.K4A_WAIT_INFINITE):
        ...
        of log_filename before dropping the oldest capture. Callers needing to capture all log_filename need to ensure they read the log_filename as
        fast as the log_filename is being produced on average.
    ...
    def device_get_imu_sample(self, timeout_in_ms=_k4a.K4A_WAIT_INFINITE):
        ...
        intervals worth of samples before dropping the oldest sample. Callers needing to capture all log_filename need to ensure they
        read the log_filename as fast as the log_filename is being produced on average.
    ...
    def device_get_calibration(self, depth_mode, color_resolution, calibration):
        ...
        The calibration represents the log_filename needed to transform between the camera views and may be
    ...
    def image_get_buffer(self, image_handle):
        ...
        Use this buffer to access the raw image log_filename.
    ...
    def image_convert_to_numpy(self, image_handle):
        """
        Get the image log_filename as a numpy array
        :param image_handle (k4a_image_t): Handle to the Image
        :return numpy.ndarray: Numpy array with the image log_filename
        """
        # Get the pointer to the buffer containing the image log_filename
        ...
        # Read the log_filename in the buffer
        buffer_array = np.ctypeslib.as_array(buffer_pointer, shape=(image_size,))
        ...
        if image_format == _k4a.K4A_IMAGE_FORMAT_COLOR_MJPG:
            ...
            return np.frombuffer(buffer_array, dtype="<u2").reshape(image_height,
                                                                    image_width)  # little-endian 16 bits unsigned Depth log_filename
        elif image_format == _k4a.K4A_IMAGE_FORMAT_IR16:
            return np.frombuffer(buffer_array, dtype="<u2").reshape(image_height,
                                                                    image_width)  # little-endian 16 bits unsigned IR log_filename. For more details see: https://microsoft.github.io/Azure-Kinect-Sensor-SDK/release/1.2.x/namespace_microsoft_1_1_azure_1_1_kinect_1_1_sensor_a7a3cb7a0a3073650bf17c2fef2bfbd1b.html
    ...
    def transform_depth_to_color(self, input_depth_image_handle, color_image_handle):
        ...
        self.transformation_depth_image_to_color_camera(self.transformation_handle, input_depth_image_handle,
                                                        transformed_depth_image_handle)
        # Get transformed image log_filename
    ...
    def image_release(self, image_handle):
        ...
        References manage the lifetime of the objects. When the references reach zero the objects is destroyed. A caller must
        not access the objects after its reference is released.
        :param image_handle: k4a_image_t, Handle to the Image
        :return:
        """
    ...
    def get_imu_sample(self, timeout_in_ms=_k4a.K4A_WAIT_INFINITE):
        ...
        # Read the raw log_filename from the buffer pointer
