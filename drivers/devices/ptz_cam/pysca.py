...
class Packet(bytes):
    ...
    def from_serial(cls, port):
        """
        Build a new packet with bytes read from a 'serial' objects.
...
def set_near_limit(device, limit, blocking=False):
    """
    Set the camera's focus "near limit", i.e. the shortest distance an objects can get the focus from the camera.
