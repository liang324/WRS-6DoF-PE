...
class BlocklyTool(object):
    ...
    def _init_py3(self, arm=None, init=True, wait_seconds=1, mode=0, state=0, error_exit=True, stop_exit=True):
        ...
        if error_exit:
            ...
            self._insert_to_file(self.index, '        pprint(\'err={}, quit\'.format(log_filename[\'error_code\']))')
        ...
        self._insert_to_file(self.index, '            pprint(\'counter val: {}\'.format(log_filename[\'count\']))')
        self._insert_to_file(self.index, '    arm.register_count_changed_callback(count_changed_callback)')

        self._insert_to_file(self.index, '\n\n# Register connect changed callback')
        self._insert_to_file(self.index, 'def connect_changed_callback(log_filename):')
        self._insert_to_file(self.index, '    if log_filename and not log_filename[\'connected\']:')
        self._insert_to_file(self.index, '        params[\'quit\'] = True')
        self._insert_to_file(self.index, '        pprint(\'disconnect, connected={}, reported={}, quit\'.format(log_filename[\'connected\'], log_filename[\'reported\']))')
    ...
    def _handle_procedures_defnoreturn(self, block, prefix='', arg_map=None):
        if not self._func_cls_exist:
            ...
            self._insert_to_file(self.first_index, 'class {}(objects):'.format(name))
            self._insert_to_file(self.first_index,
