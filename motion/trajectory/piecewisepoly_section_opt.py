...
class PiecewisePolySectionOpt(object):
    ...
    def __init__(self, method="linear"):
        """
        初始化PiecewisePolySectionOpt类
        :param method: 插值方法 ,默认为"linear"
        """
    ...
    def change_method(self, method="cubic"):
        """
        改变插值方法
        :param method: 插值方法 ,可以是"linear", "quadratic", "cubic", "quintic"
        """
        self.method = method
        if method == "linear":
    ...
    def _interpolate(self, A, samples_list, toggle_debug=False):
        """
         插值计算
        :param A: 线性回调函数(使用scipy)
        :param samples_list: 1xn_jnts的数组列表 ,每个元素包含需要插值的样本
        :param toggle_debug: 是否启用调试模式 ,默认为False
        :return: 插值后的位置、速度、加速度、抖动和x轴刻度
    ...
    def interpolate_by_max_spdacc(self,
        ...
        :param toggle_debug_fine: 是否开启精细调试模式, 默认为 False
        :param toggle_debug: 是否开启调试模式, 默认为 True
        :return: 插值后的配置
