...
class PiecewisePolyScl(object):
    """
    初始化PiecewisePolyScl类
    :param method: 插值方法 ,默认为"linear"(线性)
    """
    ...
    def change_method(self, method="cubic"):
        """
        更改插值方法
        :param method: 插值方法 ,可以是"linear"(线性)、"quadratic"(二次)、"cubic"(三次)或"quintic"(五次)
        """
        self.method = method
        if method == "linear":
    ...
    def _quadratic_solve(self):
        """
        使用二次插值创建的插值样条 ,边界条件为'clamped'(夹紧)
        :return: 插值样条
        """
    ...
    def _cubic_solve(self):
        """
        使用三次插值创建的插值样条 ,边界条件为'clamped'(夹紧)
        :return: 插值样条
        """
    ...
    def _quintic_solve(self):
        """
        使用五次插值创建的插值样条 ,边界条件为'clamped'(夹紧)
        :return: 插值样条
        """
    ...
    def _interpolate(self, A, samples_list):
        """
        插值计算
        :param A: 线性回调函数 ,因为我们使用的是scipy
        :param samples_list: 一个包含1xn_jnts的numpy数组的列表 ,每个元素包含需要插值的样本
        :return: 插值后的位置、速度、加速度、插值的x轴刻度
    ...
    def interpolate(self, control_frequency, time_intervals):
        ...
        :return: 插值后的配置、速度、加速度、插值的x轴刻度、原始x轴刻度、样本索引
        """
        self._x = [0]
        tmp_total_time = 0
        samples_list = []
    ...
    def interpolate_by_max_spdacc(self,
        ...
        :param path: 路径
        :param control_frequency: 控制频率
        :param max_vels: 路径中两个相邻姿势之间的最大关节速度, 如果为 None 则为 math.pi * 2 / 3
        :param max_accs: 路径中两个相邻姿势之间的最大关节加速度, 如果为 None 则为 math.pi
        :return: 插值后的配置
