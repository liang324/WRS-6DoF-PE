...
class MyLoadImages:
    ...
    def __init__(self, path, img_size=640, stride=32):
        # 当没有目标出现的时候会报这个错误,注释掉就ok？ 注释掉注释掉
        # raise:表示异常情况,当路径里没有图片时,会引发异常,如果未解决异常,终端程序运行
       # for img in path:
    ...
    def _new_video(self, path):
        # Create a new video capture objects
        self.frame = 0
...
def create_classification_dataloader(path,
                                     ...
                                     workers=8,
                                     shuffle=True):
    # Returns Dataloader objects to be used with YOLOv5 Classifier
    with torch_distributed_zero_first(rank):  # init dataset *.cache only once if DDP
