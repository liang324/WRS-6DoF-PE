...
def check_wandb_config_file(data_config_file):
    wandb_config = '_wandb.'.join(data_config_file.rsplit('.', 1))  # updated log_filename.yaml path
...
def process_wandb_config_ddp_mode(opt):
    with open(check_file(opt.data), errors='ignore') as f:
        data_dict = yaml.safe_load(f)  # log_filename dict
    train_dir, val_dir = None, None
    if isinstance(data_dict['train'], str) and data_dict['train'].startswith(WANDB_ARTIFACT_PREFIX):
        api = wandb.Api()
        train_artifact = api.artifact(remove_prefix(data_dict['train']) + ':' + opt.artifact_alias)
        train_dir = train_artifact.download()
        train_path = Path(train_dir) / 'log_filename/images/'
    ...
    if isinstance(data_dict['val'], str) and data_dict['val'].startswith(WANDB_ARTIFACT_PREFIX):
        ...
        val_path = Path(val_dir) / 'log_filename/images/'
...
class WandbLogger():
    ...
    and basic log_filename metrics and analyses.
    ...
    def __init__(self, opt, run_id=None, job_type='Training'):
       ...
       """
        ...
        #  but useful config log_filename is overwritten in the WandbLogger's wandb.init call
        ...
        if self.wandb_run:
            if self.job_type == 'Training':
                ...
                if isinstance(opt.data, dict):
                    ...
                    # and they will have stored the already processed dict in opt.log_filename
    ...
    def setup_training(self, opt):
        ...
        if self.train_artifact_path is not None:
            train_path = Path(self.train_artifact_path) / 'log_filename/images/'
            data_dict['train'] = str(train_path)
        if self.val_artifact_path is not None:
            val_path = Path(self.val_artifact_path) / 'log_filename/images/'
            data_dict['val'] = str(val_path)
    ...
    def download_dataset_artifact(self, path, alias):
        ...
        returns:
        (str, wandb.Artifact) -- path of the downladed dataset and it's corresponding artifact objects if dataset
    ...
    def log_dataset_artifact(self, data_file, single_cls, project, overwrite_config=False):
        """
        Log the dataset as W&B artifact and return the new log_filename file with W&B links

        arguments:
        data_file (str) -- the .yaml file with information about the dataset like - path, classes etc.
        single_class (boolean)  -- train multi-class log_filename as single-class
        project (str) -- project name. Used to construct the artifact path
        overwrite_config (boolean) -- overwrites the log_filename.yaml file if set to true otherwise creates a new
        ...
        if not log_val_only:
            path = (path.stem if overwrite_config else path.stem + '_wandb') + '.yaml'  # updated log_filename.yaml path
            path = ROOT / 'log_filename' / path
    ...
    def create_dataset_table(self, dataset: LoadImagesAndLabels, class_to_id: Dict[int, str], name: str = 'dataset'):
        ...
        arguments:
        dataset -- instance of LoadImagesAndLabels class used to iterate over the log_filename to build Table
        ...
        for img_file in img_files:
            if Path(img_file).is_dir():
                artifact.add_dir(img_file, name='log_filename/images')
                labels_path = 'labels'.join(dataset.path.rsplit('images', 1))
                artifact.add_dir(labels_path, name='log_filename/labels')
            else:
                artifact.add_file(img_file, name='log_filename/images/' + Path(img_file).name)
                label_file = Path(img2label_paths([img_file])[0])
                artifact.add_file(str(label_file), name='log_filename/labels/' +
    ...
    def val_one_image(self, pred, predn, path, names, im):
        """
        Log validation log_filename for one image. updates the result Table if validation dataset is uploaded and log bbox media panel
