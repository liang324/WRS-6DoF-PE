...
class GOFA5(ri.RobotInterface):
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), name="gofa5", enable_cc=True):
        ...
        self.camera1 = jl.JLChain(pos=pos - (0.15, -0.02, 0.015),
                                  rotmat=rotmat,
                                  homeconf=np.zeros(0),
                                  name='camera1')
        ...
        self.camera2 = jl.JLChain(pos=pos - (0.15, -0.02, 0.015),
                                  rotmat=rotmat,
                                  homeconf=np.zeros(0),
                                  name='camera2')
        ...
        self.camera3 = jl.JLChain(pos=pos - (0.15, -0.02, 0.015),
                                  rotmat=rotmat,
                                  homeconf=np.zeros(0),
                                  name='camera3')
    ...
    def get_gripper_width(self):
        import numpy as np
        # 获取左右爪尖的世界坐标
        lft_pos = self.hnd.lft_outer.jnts[-1]['gl_posq']
        rgt_pos = self.hnd.rgt_outer.jnts[-1]['gl_posq']
        # 计算欧氏距离
        width = np.linalg.norm(lft_pos - rgt_pos)
        return width
